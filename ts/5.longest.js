...
// console.log(findMaxStr('ddcdcd'));

// console.log(longestPalindrome("aacabdkacaa"));
// console.log(longestPalindrome("cadkac"));
// 2. 动态规划
function longestPalindrome(str) {
  let max = str[0];
  if(str.length < 2) return max;
  if(str.length < 3)  return str[0] === str[1] ? str : max;
  const dp = Array.from(
    {length: str.length},
  ...
  //   dp[i][i+2] = (str[i] === str[i+2]) && (str[i+1] === str[i+2-1]);
  // }
  // for(let i=0; i<str.length-3; i++) {
  //   dp[i][i+3] = (str[i] === str[i+3]) && (str[i+1] === str[i+3-1]);
  // }
